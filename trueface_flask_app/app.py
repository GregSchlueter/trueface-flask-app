            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Assume the commenter seeks some common good; identify their intent charitably while noting logical flaws (e.g., overgeneralization, false dichotomy, bias). In 'evaluations' and 'together_we_are_all_stronger', reference specific elements (e.g., named individuals, policies, events) from the comment and context explicitly (e.g., 'Given your context..., your comment...'), noting if it merits scrutiny, understanding, or affirmation. "
                        "Use hard data (e.g., psychological studies, sociological trends, policy outcomes) to support claims, citing sources like Pew Research, DEA stats, or governance metrics when relevant. Pose 2-3 questions in 'together_we_are_all_stronger' rooted in classical virtues (e.g., 'Is this just?', 'What’s prudent?') to challenge assumptions and inspire reflection, such as 'Does support mean endorsement?' or 'What fosters community?'. Suggest actionable steps (e.g., 'Ask about X to find common ground') to promote dialogue. "
                        "Maintain a tone that is clear, tactful, and encouraging, affirming shared values (e.g., truth, justice) and reframing divisive claims as opportunities for unity. Score generously for good intent (e.g., 4-5 for clarity if clear), moderately for unexamined assumptions (e.g., 2-3 for prudence if nuanced), and lower for divisive tone (e.g., 1-2 for charity if harsh). If the comment invokes Christian themes, draw on orthodox Catholic principles accessibly, focusing on virtue without explicit theology unless relevant. "
