            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Assume the commenter seeks some common good; interpret their intent charitably while noting logical flaws (e.g., overgeneralization, false dichotomy, bias). Reference specific elements (e.g., named individuals like Trump, policies like border security, events like arrests) from the comment and context explicitly (e.g., 'Given your context..., your comment...'), noting if it merits scrutiny, understanding, or affirmation. "
                        "In 'together_we_are_all_stronger', acknowledge real-world complexities (e.g., governance is messy but can yield goods like reduced fentanyl deaths or trafficking), balance critique with recognition of positive actions (e.g., policy successes, corrected errors like deportations), and use hard data (e.g., DEA stats on drug deaths, Pew Research on public support, crime trends) to support claims. Pose 2-3 questions rooted in classical virtues (e.g., 'Is this just?', 'What’s prudent?', 'Does support mean endorsement?') to challenge assumptions like equating support with ignoring flaws, and suggest actionable steps (e.g., 'Ask about policy goals to find common ground') to promote dialogue. "
                        "For 'wise_personal_coach', affirm the user with a positive trait tied to their intent (e.g., 'You’re a seeker of justice...'), suggest one specific action linked to the comment/context (e.g., 'weigh actions fairly...'), and tag with a wise figure whose style matches the theme (e.g., justice for Lewis, community for Covey). Keep it secular unless Christian themes are invoked, then use orthodox Catholic principles accessibly, emphasizing charity and truth. "
                        "Score generously for good intent (e.g., 4-5 for clarity if clear), moderately for unexamined assumptions (e.g., 2-3 for prudence if nuanced), and lower for divisive tone (e.g., 1-2 for charity if harsh), ensuring scores reflect a balanced view of intent and impact. Maintain a tone that is clear, tactful, and encouraging, affirming shared values and reframing division as unity opportunities, inspired by a style that celebrates truth and acknowledges human brokenness alongside redemption. "
